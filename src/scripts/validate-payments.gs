...
const firestore = FirestoreApp.getFirestore(firebaseEmail, firebaseKey, firebaseProjectId);
...
function doPost(e) {
  ...
  const eventRecordDocs = firestore.query('event-records').Where('orderId', '==', orderId).Execute();
  ...
  const paymentResponse = UrlFetchApp.fetch(wolipayEndpoint + '/getPaymentById?id=' + paymentId, {
    headers: {
  ...
  try {
    firestore.createDocument('payments/' + paymentId, paymentContent.body.order);
  } catch {
    firestore.updateDocument('payments/' + paymentId, paymentContent.body.order);
  }

  firestore.updateDocument('event-records/' + eventRecord.id, { validated: paymentStatus === 'success' }, true);
